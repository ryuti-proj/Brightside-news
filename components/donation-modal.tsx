...
export function DonationModal({ isOpen, onClose }: DonationModalProps) {
  ...
  const handleDonate = async () => {
    if (!isAuthenticated || !isPiUser || !piUserId || !username) {
      setStatus("error")
      setStatusMessage("Please sign in with your Pi account before donating.")
      return
    }
    ...
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
      <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl bg-white shadow-2xl border border-slate-200">
        <Card className="border-0 shadow-none bg-white">
          <CardHeader className="relative bg-white rounded-t-2xl">
            <Button variant="ghost" size="sm" className="absolute right-4 top-4 z-10" onClick={resetAndClose}>
              <X className="w-5 h-5" />
            </Button>
          ...
          <CardContent className="space-y-6 bg-white px-6 pb-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
                {PI_DONATION_PRESETS.map((preset) => (
                  <Button
                    ...
                    className={`h-16 flex flex-col ${selectedAmount === preset ? "bg-sky-600 hover:bg-sky-700 text-white" : ""}`}
                    onClick={() => {
            ...
            <div className="p-4 border border-purple-200 bg-purple-50 rounded-lg">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-purple-700">Use your Pi wallet to complete this donation inside Pi Browser.</p>
                </div>
              </div>
            </div>
